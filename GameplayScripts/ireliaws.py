...
q = {"Range": 625.0, "MinRange": 325.0, "Mana": 20}
w = {"Range": 825.0, "Mana": 70}
e = {"Range": 900.0, "Mana": 40}
r = {"Range": 1000.0}
...
def Combo(game):
    ...
    if use_q_in_combo and IsReady(game, q_spell) and game.player.mana > q["Mana"]:
        target = GetBestTargetsInRange(game, (q["Range"] or q["MinRange"]))
    ...
    if (
        ...
        and (
            getBuff(game.player, "ireliarreadybuff")
            or (getBuff(game.player, "ireliapassivecombo"))
        )
...
def winstealer_update(game, ui):
    ...
    if self.is_alive and not game.isChatOpen:
        ...
        if draw_q_range:
            # game.draw_circle_world(game.player.pos, q["Range"], 625, 1, Color.WHITE)
