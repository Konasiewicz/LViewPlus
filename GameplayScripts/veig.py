    ...
    if enemy:
        for minion in game.minions:
            if (
                ...
                and not getBuff(minion, "veigare")
            ):
...
def Combo(game):
    ...
    if (
        ...
        and q_spell.name == "veigarq3wrapper"
    ):
    ...
    if (
        ...
        if (
            target
            and not getBuff(target, "veigare")
    ...
    if use_e_in_combo and lastE + 0.3 < game.time and IsReady(game, e_spell):
        ...
        if target and not getBuff(target, "veigare"):
...
def Harass(game):
    ...
    if (
        ...
        target = GetBestTargetsInRange(game, e["Range"])
        if target and not buffIsAlive(game, getBuff(target, "veigare")):
